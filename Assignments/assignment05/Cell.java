...
public class Cell {
	...
	public void populateNeighbors(Cell[][] cells) {
		...
		if(myRow > 0) neighbors[1] = cells[myRow-1][myCol];
		if(myRow > 0 && myCol < 2) neighbors[2] = cells[myRow-1][myCol+1];
		if(myCol > 0) neighbors[3] = cells[myRow][myCol-1];
		if(myCol < 2) neighbors[4] = cells[myRow][myCol+1];
		if(myRow < 2 && myCol > 0) neighbors[5] = cells[myRow+1][myCol-1];
		if(myRow < 2) neighbors[6] = cells[myRow+1][myCol];
	...
	public boolean aliveNextTime() {
		...
		for(Cell c : neighbors) {
			if((c != null) && (c.isAlive())) counter++;
		}
		if(this.isAlive() && ((counter != 2) && (counter != 3))) return false;
		if(!(this.isAlive()) && (counter == 3)) return true;
		return this.alive;
	}
}
