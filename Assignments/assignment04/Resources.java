...
public class Resources {
	...
	private static String[] FEMALE_FIRST_NAMES = {"Mary", "Patricia", "Linda",
	...
	public static Student getStudent() {
		if(ID % 2 == 0)	{
			int i = rand.nextInt(FEMALE_FIRST_NAMES.length);
			int j = rand.nextInt(LAST_NAMES.length);
			return new Student(ID++, FEMALE_FIRST_NAMES[i] + " " + LAST_NAMES[j]);
		}
		int i = rand.nextInt(MALE_FIRST_NAMES.length);
		int j = rand.nextInt(LAST_NAMES.length);
		return new Student(ID++, MALE_FIRST_NAMES[i] + " " + LAST_NAMES[j]);
	}
}
